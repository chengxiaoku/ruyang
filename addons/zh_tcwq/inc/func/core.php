          ...
          if ($_W['role'] == 'operator') {
            ...
            //这是我自己扩展的菜单  菜单ID 为21
              $navemenu[21] = array(
                  ...
                  'items' => array(
                      0 => $this->createMainMenu('动态管理 ', $do, 'dongtai', ''),
                      1=> $this->createMainMenu('打赏记录', $do, 'dashang', ''),
                      2=> $this->createMainMenu('评论管理', $do, 'pinglun', ''),

                  )
              );
              //这是我自己扩展的菜单  菜单ID 为21
              $navemenu[22] = array(
                  'title' => '<a href="index.php?c=site&a=entry&op=display&do=renwulist&m=zh_tcwq" class="panel-title wytitle" id="yframe-22"><icon style="color:#8d8d8d;" class="fa fa-compass"></icon>任务管理</a>',
                  'items' => array(
                      0 => $this->createMainMenu('任务列表 ', $do, 'renwulist', ''),
                      1=> $this->createMainMenu('分类列表', $do, 'renwufenlei', ''),
                      //2 => $this->createMainMenu('添加任务', $do, 'addinformation', ''),
                      2=> $this->createMainMenu('任务设置', $do, 'setrenwu', ''),
                  )
              );
              ...
              $navemenu[1] = array(
                ...
                'items' => array(
                     0 => $this->createMainMenu('贴子列表 ', $do, 'information', ''),
...
if($type['is_pcfw']==1){
       ...
       public function getMainMenu2()
    {
        ...
        if($_W['isfounder'] || $_W['role'] == 'manager' || $_W['role'] == 'operator') {
            ...
            //帖子管理
             $navemenu[1] = array(
                'title' => '<a href="index.php?c=site&a=entry&op=display&do=ininformation&m=zh_tcwq" class="panel-title wytitle" id="yframe-1"><icon style="color:#8d8d8d;" class="fa fa-comment-o"></icon>  任务管理</a>',
                'items' => array(
                     0 => $this->createMainMenu('任务列表 ', $do, 'ininformation', ''),
